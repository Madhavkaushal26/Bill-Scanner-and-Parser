...
from Polygon_Editor import PolygonInteractor
...
class DocScanner(object):
    ...
    def get_contour(self, rescaled_image):
        ...
        if not approx_contours:
            print("Cannot detect screen: falling back to manual mode.")
            ...
            # Trigger interactive mode when automatic detection fails
            if self.interactive:
                screenCnt = self.interactive_get_contour(screenCnt.reshape(4,2), rescaled_image)
                return screenCnt
        else:
    ...
    def scan(self, image_path):
        ...
        orig = image.copy()
        rescaled_image = GU.resize(image, height = int(RESCALED_HEIGHT))
        ...
        warped = GU.four_point_transform(orig, screenCnt * ratio)
